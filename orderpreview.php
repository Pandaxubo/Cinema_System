...
$senior = $_POST['senior'];
$seats = $_POST['seats'];
$_SESSION['seatsdetail'] = $seats;
$promotion = $_POST['promo'];
$totalseats = intval($children[0]) + intval($adult[0]) +intval($senior[0]);
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col" id="orderinfo">
                <h3>Your Order</h3>
                <hr>
                <p>Ticket count: <?php echo $totalseats;?></p>
                <p>Children:<?php echo $children[0];?></p>
                <p>Adult:<?php echo $adult[0];?></p>
                <p>Senior:<?php echo $senior[0];?></p>
                <p>Room:<?php echo $_SESSION['showroom'];?></p>
                <p>Date:<?php echo $_SESSION['showdate'];?></p>
                <p>Time:<?php echo $_SESSION['showtime'];?></p>
                <p>Seats:
                <?php foreach ($_POST['seats'] AS $seats):?>
                    <?php echo $seats;?>
                <?php endforeach;?>
                <br>
                <p>Promotion code:<?php echo $promotion;?></p>
                <p>Discount:10%</p>
                <?php $total = (intval($children[0]) * 5 + intval($senior[0]) * 8 + intval($adult[0]) * 10) * 0.9 ;?>
                <hr>
                <p class="float-right">Subtotal: <?php echo $total;?></p>
                <br><br>
                <button type="button" onclick="window.location.href = 'bookmovie.php'" class="btn btn-primary float-left">Edit Order</button>
                <button type="button" onclick="window.location.href = 'homepage.html'" class="btn btn-secondary">Cancel Order</button>
                <button type="button" onclick="window.location.href = 'checkout.html'" class="btn btn-outline-success float-right">Next -></button>
